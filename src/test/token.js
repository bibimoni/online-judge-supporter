class TokenType {
     ...
     */
    static #isInt(token) {
        return (this.#validateNumber(token) && TokenType.#int_token_regex.test(token));
    }
    /**
     * not an isFinite float, have the form of d.d
     */
    static #isFloat(token) {
        return (this.#validateNumber(token) && TokenType.#float_token_regex.test(token));
    }
    ...
    static #isUnknown(token) {
        return (token.length === 0 ||
            (!this.#isInt(token) && !this.#isFloat(token) && !this.#isString(token)));
    }
    ...
    static #cmpFloat(token1, token2) {
        return (TokenType.#sameType(token1, token2) &&
            Math.abs(token1.token - token2.token) < TokenType.#float_token_epsilon);
    }
