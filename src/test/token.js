class TokenType {
  ...
  static #isInt(token) {
    return (
      this.#validateNumber(token) && TokenType.#int_token_regex.test(token)
    );
  }
  ...
  static #isFloat(token) {
    return (
      this.#validateNumber(token) && TokenType.#float_token_regex.test(token)
    );
  }
  ...
  static #cmpFloat(token1, token2) {
    return (
      TokenType.#sameType(token1, token2) &&
